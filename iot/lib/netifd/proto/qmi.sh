...
handle_raw_ip_mode() {
	local modem_bus="$1"
	local ifname raw_ip
	ifname=$(uci -q get network.${modem_bus}.ifname)
	raw_ip=$(uci -q get network.${modem_bus}.raw_ip)
...
proto_qmi_setup() {
	...
	. /lib/functions/gl_util.sh;
        ...
        [ "$model" = "mt300n-v2" ] && {
	  handle_raw_ip_mode  "$interface"
	  dataformat="$(uqmi -s -d "$device" --wda-get-data-format)"
	  if [ "$dataformat" = '"raw-ip"' ]; then
		if [ -f /sys/class/net/$ifname/qmi/raw_ip ]; then
			logger -t modem "Device does not support 802.3 mode. Informing driver of raw-ip only for $ifname .."
			echo "Y" > /sys/class/net/$ifname/qmi/raw_ip
		else
			logger -t modem "Device only supports raw-ip mode but is missing this required driver attribute: /sys/class/net/$ifname/qmi/raw_ip"
		fi
	  fi
	}
	...
	if [ -n "$node" ];then
                modem_bus=$(echo $node|cut -d ':' -f 1)
        else
                modem_bus=`uci get glconfig.modem.bus 2>/dev/null`
        fi

	[ `cat /tmp/sim_type` = "verizon" ] && pdptype="ipv4v6"
	if [ "$ipv6_enabled" = "1" -a "$ipv6_wan_interface" = "$interface" ];then
		pdptype="ipv4v6"
		[ `cat /tmp/sim_status` = "ready" -a `cat /tmp/sim_type` != "verizon" ] && gl_modem -B $modem_bus AT AT+CGDCONT=1,\"IPV4V6\"
	fi
	...
	#fix qmi
	autoconnect=""
	[ `cat /tmp/sim_type` = "verizon" ] && profile=3 || profile=1
	...
	[ "$ipv6_enabled" = "1" -a "$ipv6_wan_interface" = "$interface" ] && [ "$pdptype" = "ipv6" -o "$pdptype" = "ipv4v6" ] && {
