        ...
        mounted: function mounted() {
            ...
            if (that.software.packages) {
                ...
                that.$store.dispatch("call", {
                    ...
                    if (result.success) {
                        if (result.success && result.flash.flash_total != 0) {
                            that.flash_free = result.flash.flash_free / result.flash.flash_total;
                            that.flash_free = that.flash_free.toFixed(2) * 100;
                            that.flash_total = Math.floor(result.flash.flash_free / 1024);
                        } else {
                            that.flash_total = "0"
                            that.flash_free = "0"
                        }
            ...
            setTimeout(function () {
                that.$store.dispatch('call', {
                    api: "findsoftwareapplist",
                }).then(function (result) {
                    if (result.user_apps) {
                        that.softAppStatus = true;
                        var list = result.user_apps.split(' ');
                        var data = {};
                        for (var i = 0; i < list.length; i++) {
                            data = {};
                            data.name = list[i];
                            data.id = i;
                            that.softAppList.push(data);
                            that.checkList.push(data.name);
                        }
                    }
                });
            }, 500);
        ...
        watch: {
            'checkList': {
                handler: function (val, oldVal) {
                    if (val.length === this.softAppList.length) {
                        this.checked = true;
                    } else {
                        this.checked = false;
                    }
        ...
        methods: {
            checkedAll: function checkedAll() {
                ...
                if (that.checked) {
                    ...
                    that.checkList = [];
                    that.softAppList.forEach(function (item, index) {
                        that.checkList.push(item.name);
                    });
                }
            },
            reinstall: function reinstall() {
                var that = this;
                ...
                var item = ""
                if (this.checkList) {
                    item = this.checkList.join(" ");
                }
                item = item.replace(/(\s*$)/g, '');
                this.$store.dispatch('call', {
                    api: "findsoftwarereinstall",
                    data: {
                        user_apps: item
                    },
                    timeOut: 120000
            ...
            getInstalledSortWare: function getInstalledSortWare() {
                ...
                this.$store.dispatch("call", {
                    ...
                    if (result.success) {
                        if (result.flash.flash_total != 0) {
                            if (result.flash.flash_free) {
                                that.flash_free = result.flash.flash_free / result.flash.flash_total;
                                that.flash_free = (that.flash_free * 100).toFixed(1);
                                that.flash_total = Math.floor(result.flash.flash_free / 1024);
                            }
                        } else {
                            that.flash_total = "0"
            ...
            fuzzyQuery: function fuzzyQuery(list, prop, keyWord) {
                var arr = [];
                for (var i = 0; i < list.length; i++) {
                    if (list[i][prop].toLowerCase().indexOf(keyWord.toLowerCase()) >= 0) {
                        arr.push(list[i]);
                    }
                }
                return arr;
            ...
            installsofeware: function installsofeware(item, index) {
                ...
                that.$store.dispatch("call", {
                    ...
                    if (result.success) {
                        ...
                        that.$store.dispatch('getList_vpn');
                    } else {
                        that.errMsg = result.stderr;
                        that.status = "failed";
                        // that.modalClose();
