    ...
    var vueComponent = Vue.extend({
        ...
        computed: {
            ...
            btnStatus: function btnStatus() {
                if (!this.countryCode || !this.modalMsg) {
                    return true;
                }
                return false;
            },
        ...
        mounted: function mounted() {
            ...
            that.$store.dispatch("call", { api: 'smscode', data: { action: "get" } }).then(function (result) {
                if (result.success) {
                    ...
                    that.countryCode = result.country_code ? result.country_code : '+93';
                }
            });
        ...
        methods: {
            ...
            removeMsg: function removeMsg(item, index, event) {
                ...
                _this.$store.commit("showModal", {
                    show: true,
                    title: "Caution",
                    message: _this.t('Confirm deletion information?'),
                    cb: function cb() {
            ...
            deleteAllMessage: function deleteAllMessage() {
                ...
                that.$store.commit("showModal", {
                    show: true,
                    title: "Caution",
                    message: that.t('Confirm deletion information?'),
                    cb: function cb() {
                        that.$store.dispatch("call", {
                            api: "delete_all_message", data: {
                                modem_id: that.moid ? that.moid : 255
                            }
                        }).then(function (result) {
                            if (result.success) {
                                that.$message({
