    ...
    var vueComponent = Vue.extend({
        ...
        computed: {
            ...
            ipconflict: function ipconflict() {
                var item;
                if (this.waninfo.gateway && this.router.ip_addr == this.waninfo.gateway && this.waninfo.code != -6) {
                    return item = 'lanipconflict';
                } else if (this.waninfo.gateway && this.router.guest_addr == this.waninfo.gateway && this.waninfo.code != -6) {
            ...
            // errorMsg(){
            //     return  this.logStatus && this.logStatus
            // }
        },
        beforeDestroy: function beforeDestroy() {
            this.timer && clearInterval(this.timer);
            this.tiemr_waninfo && clearTimeout(this.tiemr_waninfo);
        },
        mounted: function mounted() {
            var that = this;
            if (!this.waninfo.ip) {
                if (this.waninfo.proto) {
                    if (this.waninfo.proto == 'static') {
                        this.wanType = this.dataList[this.prolist.indexOf('dhcp')];
                    } else {
                        this.wanType = this.dataList[this.prolist.indexOf(this.waninfo.proto)];
                    }
                    if (this.waninfo.log) {
                        that.logStatus = this.waninfo.log.split('\n');
                    } else {
                        that.logStatus = [];
                    }
                    this.show_wan = true;
            ...
            this.tiemr_waninfo = setTimeout(function () {
                that.renderData("render");
            }, 900);
        },
        methods: {
            // 第一次获取 wan info
            renderData: function renderData(target) {
                ...
                if (nocache) {
                    that.$store.dispatch("call", { api: "waninfo" }).then(function (result) {
                        if (result.log) {
                            that.logStatus = result.log.split('\n');
                        } else {
                            that.logStatus = [];
                        }
                        if (result.code == "-17") {
                        ...
                        if (result.success) {
                            ...
                            if (result.ip) {
                                ...
                                if (that.router.model != "sf1200") {
                                    if (that.waninfo.cableinwan) {
                                        that.getWanType(result);
                                    }
                                }else{
            ...
            timerData: function timerData(itime) {
                ...
                that.timer = setInterval(function () {
                    that.$store.dispatch("call", { api: "waninfo" }).then(function (result) {
                        ...
                        that.timerData(itime);
                    }).catch(function() {
                        that.timerData(itime);
                    });
                }, itime);
            },
            getWanType: function getWanType(result) {
                var that = this;
                that.timer && clearInterval(that.timer);
                if (result.success) {
                    var waninfo = $.extend(true, {}, that.waninfo);
                ...
                if (this.btnName == "Modify") {
                    ...
                    if (that.wanType == "PPPoE") {
                        ...
                        if (this.waninfo.ip && this.waninfo.proto == "dhcp") {
                            this.$message({
                                type: "warning",
                                msg: this.t('Connected successfully without having to reconnect')
                            });
                            that.btnName = "Modify";
                            that.changeAp = true;
                            return;
            ...
            check: function check(data) {
                ...
                that.$store.dispatch("call", {
                    ...
                    if (result.success) {
                        ...
                        that.$store.dispatch("call", { api: 'router_mini' });
                        that.renderData();
                    } else {
                        that.$message({
                            type: "error",
