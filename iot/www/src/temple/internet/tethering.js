    ...
    var vueComponent = Vue.extend({
        ...
        computed: {
            ...
            ipconflict: function ipconflict() {
                let ipconflict = null
                if ( this.teinfo.gateway && this.router.ip_addr ==  this.teinfo.gateway &&  this.teinfo.code != -6) {
                    ipconflict = 'lanipconflict';
                } else if ( this.teinfo.gateway && this.router.guest_addr ==  this.teinfo.gateway &&  this.teinfo.code != -6) {
                    ipconflict =  'guestipconflict';
                } 
                return ipconflict
            },
        ...
        methods: {
            ...
            getteData: function getteData(render, isconnecting, code) {
                var that = this;
                // var nocache = true;
                if (render) {
                ...
                that.$store.dispatch("call", { api: "teinfo", aysnc: true }).then(function (result) {
                    ...
                    } else if (result.code == "-17" || that.router.model === 'sf1200') { // sf1200 无tethering 模块
                        that.$store.commit("removeInter", { data: "tethering" });
                        clearTimeout(that.timert);
                        return;
                    } else {
                        that.$store.commit("setofflist", { data: "tethering" });
                    }
                    that.timerTeData(isconnecting, code);
                }).catch(function() {
                    that.timerTeData(isconnecting, code);
                })
            },
            ...
            timerTeData: function timerTeData(isconnecting, code) {
                ...
                this.timert = setTimeout(function () {
                    that.getteData(false, isconnecting, code);
