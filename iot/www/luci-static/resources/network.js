...
return null;},this));},getNetworks:function(){return initNetworkState().then(L.bind(enumerateNetworks,this));},deleteNetwork:function(name){var requireFirewall=Promise.resolve(L.require('firewall')).catch(function(){}),network=this.instantiateNetwork(name);return Promise.all([requireFirewall,initNetworkState()]).then(function(res){var uciInterface=uci.get('network',name),firewall=res[0];if(uciInterface!=null&&uciInterface['.type']=='interface'){return Promise.resolve(network?network.deleteConfiguration():null).then(function(){uci.remove('network',name);uci.sections('luci','ifstate',function(s){if(s.interface==name)
...
uci.unset('wireless',s['.name'],'network');});if(firewall)
...
return(a.metric-b.metric);if(a.interface<b.interface)
return-1;else if(a.interface>b.interface)
return 1;return 0;});return rv;},this));},getStatusByAddress:function(addr){return initNetworkState().then(L.bind(function(){var rv=[];for(var i=0;i<_state.ifaces.length;i++){if(Array.isArray(_state.ifaces[i]['ipv4-address']))
...
return rv.sort(function(a,b){return a[0]>b[0]});}});Protocol=baseclass.extend({__init__:function(name){this.sid=name;},_get:function(opt){var val=uci.get('network',this.sid,opt);if(Array.isArray(val))
...
else{var ifnames=L.toArray(uci.get('network',this.sid,'ifname'));for(var i=0;i<ifnames.length;i++){var m=ifnames[i].match(/^([^:/]+)/);return((m&&m[1])?Network.prototype.instantiateDevice(m[1],this):null);}
...
return true;}
return false;},deleteConfiguration:function(){}});Device=baseclass.extend({__init__:function(ifname,network){var wif=getWifiSidByIfname(ifname);if(wif!=null){var res=getWifiStateBySid(wif)||[],netid=getWifiNetidBySid(wif)||[];this.wif=new WifiNetwork(wif,res[0],res[1],netid[0],res[2],{ifname:ifname});this.ifname=this.wif.getIfname();}
...
return this.networks;},getWifiNetwork:function(){return(this.wif!=null?this.wif:null);}});WifiDevice=baseclass.extend({__init__:function(name,radiostate){var uciWifiDevice=uci.get('wireless',name);if(uciWifiDevice!=null&&uciWifiDevice['.type']=='wifi-device'&&uciWifiDevice['.name']!=null){this.sid=uciWifiDevice['.name'];}
